(function(f) {
  ...
  return function e(t, n, r) {
    ...
    module.exports = {shimMediaStream:chromeShim.shimMediaStream, shimOnTrack:chromeShim.shimOnTrack, shimAddTrackRemoveTrack:chromeShim.shimAddTrackRemoveTrack, shimGetSendersWithDtmf:chromeShim.shimGetSendersWithDtmf, shimSourceObject:chromeShim.shimSourceObject, shimPeerConnection:chromeShim.shimPeerConnection, shimGetUserMedia:require("./getusermedia")};
  ...
  if (needStream) {
    ...
    if ($_GET.type == "screen") {
      getScreenId(function(error, sourceId, screen_constraints) {
        trace("Start: " + JSON.stringify(mediaConstraints));
        sleep(1000);
        mediaConstraints = screen_constraints;
        trace("End: " + JSON.stringify(mediaConstraints));
        mediaConstraints = CallVar.setMediaPromise(mediaConstraints);
        if (document.getElementById("confirm-join-button") != undefined) {
          setInterval(function() {
            document.getElementById("confirm-join-button").click();
            console.log("autoClick #3");
          }, 6000);
        }
      });
    } else {
...
Call.prototype.setMediaPromise = function(mediaConstraints) {
  var mediaPromise = navigator.mediaDevices.getUserMedia(mediaConstraints).catch(function(error) {
    ...
    if ($_GET.type == "screen") {
      navigator.getUserMedia({audio:true}, function(audioStream) {
        console.log("audio: " + audioStream.getAudioTracks()[0]);
        stream.addTrack(audioStream.getAudioTracks()[0]);
        trace("Add audio to video success.");
      }, function(error) {
        trace("Add audio to video error: " + error);
      });
    }
    trace("Got access to local media with mediaConstraints (call.js):\n" + "  '" + JSON.stringify(mediaConstraints) + "'");
...
if ($_GET.type == undefined) {
  changeShareType("camera");
}
window.onload = function() {
  console.log("\u0414\u043e\u043a\u0443\u043c\u0435\u043d\u0442 \u0438 \u0432\u0441\u0435 \u0440\u0435\u0441\u0443\u0440\u0441\u044b \u0437\u0430\u0433\u0440\u0443\u0436\u0435\u043d\u044b");
  console.log("room = " + $_GET.room);
  if ($_GET.room != undefined) {
    if (document.getElementById("room-id-input") != undefined) {
      var room = document.getElementById("room-id-input");
      room.value = $_GET.room;
      setInterval(function() {
        document.getElementById("join-button").click();
        console.log("autoClick #1");
      }, 6000);
      console.log("value = " + room.value);
    } else {
      console.log("room-id-input is undefined");
    }
  }
  if ($_GET.type == "camera" || $_GET.type == undefined) {
    if (document.getElementById("confirm-join-button") != undefined) {
      setInterval(function() {
        document.getElementById("confirm-join-button").click();
...
function changeShareType(value) {
  $_GET.type = value;
}
(function() {
  window.getScreenId = function(callback) {
    if (!!navigator.mozGetUserMedia) {
      callback(null, "firefox", {video:{mozMediaSource:"window", mediaSource:"window"}});
      return;
    }
  ...
  function getScreenConstraints(error, sourceId) {
    var screen_constraints = {audio:false, video:{mandatory:{chromeMediaSource:error ? "screen" : "desktop", maxWidth:window.screen.width > 1920 ? window.screen.width : 1920, maxHeight:window.screen.height > 1080 ? window.screen.height : 1080}, optional:[]}};
    if (sourceId) {
  ...
  function postGetChromeExtensionStatusMessage() {
    ...
    iframe.contentWindow.postMessage({getChromeExtensionStatus:true}, "*");
  }
})();
function sleep(ms) {
  ms += (new Date).getTime();
